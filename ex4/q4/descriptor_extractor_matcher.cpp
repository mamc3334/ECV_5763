// Original: ECV-ECEE-5763-mcgaffin/sift/descriptor_extractor_matcher.cpp
// Modified by Mason McGaffin for ECV 5763 OpenCV 4
...
void help(char** argv)
{
    cout << "\nThis program demonstrats keypoint finding and matching between 2 images using features2d framework.\n"
     ...
     << "Example of case1:\n"
     << "./descriptor_extractor_matcher ORB ORB BruteForce NoneFilter cola.jpg 0\n"
     << "\n"
     ...
     << "\n"
     << "Possible detectorType values: ORB, SIFT, BRISK, AKAZE, etc.\n"
     << "Possible descriptorType values: ORB, SIFT, BRISK, AKAZE, etc.\n"
     << "Possible matcherType values: see in documentation on DescriptorMatcher::create().\n"
...
void doIteration( const Mat& img1, Mat& img2, bool isWarpPerspective,
                  ...
                  Ptr<Feature2D>& detector, Ptr<Feature2D>& descriptorExtractor,
                  ...
                  double ransacReprojThreshold, RNG& rng )
{
    ...
    if( !H12.empty() && eval )
    {
        cout << "< Evaluate feature detector expression (Unsupported in OpenCV 4)..." << endl;
        cout << ">" << endl;
    }
    ...
    if( !H12.empty() && eval )
    {
        cout << "< Evaluate descriptor matcher expression (Unsupported in OpenCV 4)..." << endl;
        cout << ">" << endl;
    }
...
int main(int argc, char** argv)
{
    ...
    Ptr<Feature2D> detector = string(argv[1]) == "ORB" ? ORB::create().staticCast<Feature2D>() : (string(argv[1]) == "SIFT" ? SIFT::create().staticCast<Feature2D>() : (string(argv[1]) == "BRISK" ? BRISK::create().staticCast<Feature2D>() : (string(argv[1]) == "AKAZE" ? AKAZE::create().staticCast<Feature2D>() : Ptr<Feature2D>())));
    Ptr<Feature2D> descriptorExtractor = string(argv[2]) == "ORB" ? ORB::create().staticCast<Feature2D>() : (string(argv[2]) == "SIFT" ? SIFT::create().staticCast<Feature2D>() : (string(argv[2]) == "BRISK" ? BRISK::create().staticCast<Feature2D>() : (string(argv[2]) == "AKAZE" ? AKAZE::create().staticCast<Feature2D>() : Ptr<Feature2D>())));
