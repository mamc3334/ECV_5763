...
def parse_jetson_csv(file_path):
    ...
    # Extracts GPU percentage after GR3D_FREQ,
    ...
    return timestamps, overall_cpu_array, cpu_matrix, gpu_array, ram_used_arr, ram_total_arr


def plot_metrics(timestamps, overall_cpu, cpu_matrix, gpu_usage, ram_used, ram_total):
    if not timestamps:
    ...
    # Figure 1: Overall CPU & GPU Usage
    ...
    ax2.plot(elapsed_seconds, gpu_usage, label="GPU Usage (%)", linewidth=1.5, color="tab:orange")
    ax2.fill_between(elapsed_seconds, gpu_usage, color="tab:orange", alpha=0.4)
    ax2.set_title("Overall GPU Utilization")
    ...
    fig.tight_layout()
    fig.savefig("overall_CPU_GPU.png", dpi=300)
    print("Successfully saved: overall_CPU_GPU.png")
...
if __name__ == "__main__":
    ...
    times, cpu_avg, cpu_cores, gpu, ram, ramT  = parse_jetson_csv(csv_filename)
    plot_metrics(times, cpu_avg, cpu_cores, gpu, ram, ramT)
